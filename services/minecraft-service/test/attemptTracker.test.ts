...
describe('AttemptTracker', () => {
  it('start emits AttemptStarted with the embedded roster on the Attempt aggregate', () => {
    const h = harness()
    const envelope = h.tracker.start({ label: 'drill-1', difficulty: 'easy', teams: TEAMS })
  ...
  it('milestones fire once per team, dedupe within the attempt, and ignore non-roster villagers', () => {
    const h = harness()
    h.tracker.start({ label: null, difficulty: 'easy', teams: TEAMS })
  ...
  it('milestone envelopes point at their source event (causationId) and share its correlationId', () => {
    const h = harness()
    h.tracker.start({ label: null, difficulty: 'easy', teams: TEAMS })
  ...
  it('the iron_pickaxe milestone records the win with the ledger-proof pointer', () => {
    const h = harness()
    h.tracker.start({ label: null, difficulty: 'normal', teams: TEAMS })
  ...
  it('end{won} carries the winning pointers and honest-race deltas, then clears the attempt', () => {
    const h = harness()
    h.tracker.start({ label: null, difficulty: 'normal', teams: TEAMS })
  ...
  it("end{won} without a recorded winning milestone is refused — 'won' is a ledger claim", () => {
    const h = harness()
    h.tracker.start({ label: null, difficulty: 'normal', teams: TEAMS })
  ...
  it('an aborted attempt reports null winners even when a win was recorded', () => {
    const h = harness()
    h.tracker.start({ label: null, difficulty: 'easy', teams: TEAMS })
  ...
  it('observing with no attempt running is a no-op; double-start is refused', () => {
    const h = harness()
    h.tracker.observe(gathered('red-1', 'coal_ore'))
    expect(h.published).toEqual([])
    h.tracker.start({ label: null, difficulty: 'easy', teams: TEAMS })
    expect(() => h.tracker.start({ label: null, difficulty: 'easy', teams: TEAMS })).toThrow(/already running/)
  })

  it('derived events never re-derive: a ProgressionMilestone observed back is silent', () => {
    const h = harness()
    h.tracker.start({ label: null, difficulty: 'easy', teams: TEAMS })
    h.tracker.observe(gathered('red-1', 'coal_ore'))
